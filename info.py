...
PICS = (environ.get('PICS', 'https://envs.sh/Aku.jpg')).split()


# Admins & Users
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '7202724594 6762558871').split()] # For Multiple Id Use One Space Between Each.
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '7202724594 6762558871').split()]  # For Multiple Id Use One Space Between Each.
...
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002573533031').split()]  # For Multiple Id Use One Space Between Each.
...
auth_channel = environ.get('AUTH_CHANNEL', '-1002634896858') # give your force subscribe channel id here else leave it blank
AUTH_CHANNEL = int(auth_channel) if auth_channel and id_pattern.search(auth_channel) else None

# This Channel Is For When User Request Any File Name With command or hashtag like - /request or #request
reqst_channel = environ.get('REQST_CHANNEL', '-1002634896858')
...
support_chat_id = environ.get('SUPPORT_CHAT_ID', '-1002201458256')
SUPPORT_CHAT_ID = int(support_chat_id) if support_chat_id and id_pattern.search(support_chat_id) else None

# This Channel Is For /batch command file store.
FILE_STORE_CHANNEL = [int(ch) for ch in (environ.get('FILE_STORE_CHANNEL', '-1002634896858')).split()]  # For Multiple Id Use One Space Between Each.
...
DELETE_CHANNELS = [int(dch) if id_pattern.search(dch) else dch for dch in environ.get('DELETE_CHANNELS', '-1002634896858').split()]  # For Multiple Id Use One Space Between Each.


# MongoDB information
DATABASE_URI = environ.get('DATABASE_URI', "mongodb+srv://snowie2122:snowie@filetolink.7imij.mongodb.net/?retryWrites=true&w=majority&appName=Filetolink")   # IF Multiple Database Is False Then Fill Only This Database Url.
...
# Links
GRP_LNK = environ.get('GRP_LNK', 'https://t.me/downloaderzonefreemirrorleech')
CHNL_LNK = environ.get('CHNL_LNK', 'https://t.me/filtergroupupdates')
SUPPORT_CHAT = environ.get('SUPPORT_CHAT', 'DZONEDISCUSSION') # Support Chat Link Without https:// or @
OWNER_LNK = environ.get('OWNER_LNK', 'https://t.me/ANSHtheGENIOUS')
...
VERIFY_SHORTLINK_API = environ.get('VERIFY_SHORTLINK_API', 'a5f363bebec3e3ddf5c49b11f147638a68fdcc5f')
